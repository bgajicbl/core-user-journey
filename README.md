...
- **Backend** — Spring Boot 3 (Java 21), layered as `web` (REST controllers + DTOs) → `domain`/`repository` (JPA entities/Spring Data) → `security` (JWT issuing/validation). Data lives in an H2 in-memory database, seeded with the sample courses/lessons from the brief on startup.
